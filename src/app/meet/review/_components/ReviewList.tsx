  ...
  return (
    <div>
      ...
      <div className="space-y-3">
        {reviews.length > 0 &&
          reviews.map((review) => (
            <div key={review.id} className="rounded-xl border p-4 bg-gray-100 space-y-1">
              <div className="flex justify-between">
                <div className="flex items-center gap-2">
                  <Image src="/default-profile.png" alt="profile" width={24} height={24} className="rounded-full" />
                  <span className="font-medium">{review.user_name}</span>
                </div>
                <span className="text-xs text-gray-500">{review.created_at.split('T')[0]}</span>
              </div>
      ...
      {modals.reviewEdit && (
        <ReviewEditModal
          review={modalData.reviewEdit as { id: number; rating: number; content: string }}
