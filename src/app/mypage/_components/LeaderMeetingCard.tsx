...
const LeaderMeetingCard = ({ meeting }: LeaderMeetingCardProps) => {
  const { application_deadline, title, date, area, current_people, meet_rating, session_count } = meeting;
  ...
  return (
    <div className="rounded-xl border bg-white space-y-3 p-5 shadow-sm text-xs w-full">
      <div className="flex justify-between items-center">
        <div className="space-y-3">
          ...
          <div className="flex flex-wrap gap-x-4 gap-y-1 text-gray-600 text-base">
            <div>
              <span className="font-medium text-gray-700">활동 일시</span> {date}
            </div>
            <div>
              <span className="font-medium text-gray-700">장소</span> {area}
            </div>
            <div>
              <span className="font-medium text-gray-700">모임 횟수</span> {session_count}회
            </div>
          </div>
        </div>
        <MoreLinkButton href={`/meet/${meeting.id}`}>상세 보기</MoreLinkButton>
      </div>
      ...
      <div className="flex justify-between items-center rounded-md p-3 text-xs bg-orange-50">
        {meeting.status === '완료' ? (
          ...
          <div className="flex gap-4 items-center text-accent-main font-medium text-xs">
            <div className="flex items-center gap-1">
              ...
              <span className="text-black text-xs">{current_people}명</span>
            </div>
            <div className="flex items-center gap-1">
        ...
        {/* 우측 버튼 */}
        <MoreLinkButton href={meeting.status === '완료' ? '/review' : '/applicants'}>
          {meeting.status === '완료' ? '참여자 후기 보기' : '신청자 정보 보기'}
        </MoreLinkButton>
      </div>
    </div>
  );
